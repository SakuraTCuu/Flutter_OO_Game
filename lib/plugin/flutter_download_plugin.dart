...
import 'package:flutter_huoshu_app/plugin/download_ios_plugin.dart';

import 'download_ios_impl.dart';
...
class FlutterDownloadPlugin {
  ...
  //开始一个任务  文件名/url
  static startTask(String fileName, String fileUrl, int gameId, {int isOfficial}) {
    if (Platform.isIOS) {
      DownloadIosPlugin.startTask(fileName, fileUrl, gameId);
    } else {
  ...
  static Future<int> getStatus(
    ...
    if (Platform.isIOS) {
      return DownloadIosPlugin.getStatus(fileName, fileUrl, game);
    }
  ...
  static Future install(String fileName, String fileUrl) async {
    if (Platform.isIOS) {
      return DownloadIosPlugin.install(fileName, fileUrl);
    }
  ...
  static Future open(String packageName) async {
    if (Platform.isIOS) {
      return DownloadIosPlugin.open(packageName);
    }
...
class DownloadStatus {
  ...
  static const int REQUEST = 108; //请求中
}
