...
class UserService {
  ...
  //获取分享
  static Future<ShareInfo> getShareDataByApp({String type = 'app', num lotteryId}) async {
    dynamic response =
    await CommonDio.post("app/share/detail", data: {"share_type": type, if (null != lotteryId) "object_id": lotteryId});
  ...
  static Future<LotteryIndexData> getLotteryIndex(int page, int offset) async {
    dynamic response = await CommonDio.post("app/lottery/index",
        data: {"page": page, "offset": offset,"act_id": 6,});
    return LotteryIndexData.fromJson(response);
  }

  //抽奖主页
  static Future<LotteryDrawData> getLotteryDraw() async {
    dynamic response = await CommonDio.post("app/lottery/draw",
        data: {"act_id": 6,});
    return LotteryDrawData.fromJson(response);
  }

  //抽奖实物商品
  static Future lotteryRealGoods(String goodsId,String phone,String name,String address) async {
    dynamic response = await CommonDio.post("app/lottery/get_reward",
        data: {"order_id": goodsId, "num": 1,"address" :address,"mobile" : phone,"consignee" : name});
    return response;
  }

  //获取兑换记录
  static Future<ExchangeRecordListData> getExchangeRecordList(int page, int offset) async {
  ...
  static Future<ExchangeDetail> getExchangeRecordDetails(String id) async {
    dynamic response = await CommonDio.post("app/shop/goods/exchange_detail",
        data: {"order_id": id,});
  ...
  static Future notifyShare(String shareId, String target) async {
    dynamic response = await CommonDio.post("app/share/notify",
        data: {"share_id": shareId, "to_target": target, "share_type": "app"},isShowError: false);
    return response;
  }

  //获取积分商城列表
  static Future<GoodsListData> getGoods(int page, int offset,int isReal) async {
  ...
  //兑换商品
  static Future exchangeRealGoods(int goodsId,String phone,String name,String address) async {
    dynamic response = await CommonDio.post("app/shop/goods/exchange",
        data: {"goods_id": goodsId, "num": 1,"address" :address,"mobile" : phone,"consignee" : name});
    return response;
  }
...
//    dynamic response = await CommonDio.post("app/recycle/explain");
  ...
  static Future<RecycleList> addRecycle(int mgMemId) async{
    dynamic response = await CommonDio.post("app/account/recycle/add_recycle",data: {
      "mg_mem_id":mgMemId,
      "recycle_way":1
    });
    return RecycleList.fromJson(response);
  ...
  static Future<RecycleList> getRecycleOrderList(int page) async{
    dynamic response = await CommonDio.post("app/account/recycle/order_list",data: {
      "page":page,
      "offset":10,
    });
    return RecycleList.fromJson(response);
  ...
  static Future<RecycleProOrder> recycleProOrder(int id) async {
    dynamic response = await CommonDio.post("app/account/recycle/pre_order",data: {
      "recycle_id":id,
    });
  ...
  //小号赎回支付
  static Future<DealPayInfoData> recyclePay(String orderId,String payWay) async {
  ...
  static Future recycleQueryOrder(String orderId) async {
    dynamic response = await CommonDio.post("app/account/recycle/query_order",data: {
      "order_id":orderId,
    });
    return response;
  }
  ...
  //游戏币充值
  static Future<DealPayInfoData> cpsPay(String payWay, num amount,String gameId,String userName) async {
    dynamic response = await CommonDio.post("ncps/app/order/pay", isShowDialog: true,
        data: {"payway": payWay, "amount": amount,"game_id":gameId,"username":userName});
    return DealPayInfoData.fromJson(response);
  }
  ...
  static Future<DealPayInfoData> cpsGamePay(
    ...
    dynamic response = await CommonDio.post("ncps/app/order/pay", isShowDialog: true, data: {
      "payway": payWay,
      "amount": amount,
      "game_id": gameId,
      "username": userName,
  ...
  //CPS充值账户校验
  static Future<CpsRateCheck> cpsAccountCheck(num gameId, String account, num cpId) async {
